...
public class ReplaceColorCommand extends BaseCommand {
	...
	public ReplaceColorCommand(Point clickedPixel, Paint currentPaint) {
		super(currentPaint);
	...
	public void run(Canvas canvas, Bitmap bitmap) {
		...
		if (PaintroidApplication.savedPictureUri == null
			...
			Log.w(PaintroidApplication.TAG,
					"ReplaceColor Command color: " + mPaint.getColor());
		} else {
			int colorToReplace = bitmap.getPixel(mClickedPixel.x,
					mClickedPixel.y);

			try{
				JniBitmap.floodFillAnywhere(bitmap, mClickedPixel.x,
						mClickedPixel.y, mPaint.getColor(), (int) SELECTION_THRESHOLD);
			}catch(Throwable e){
				//native floodfill did not work
				Log.e("Floodfill", "Fallback to java floodfill_anywhere: native floodfill_anywhere error. Probably you did not include the native libaries. "+e.getMessage());
